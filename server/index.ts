...
(async () => {
  ...
  try {
    ...
    await db.execute(sql`ALTER TABLE platform_settings ADD COLUMN IF NOT EXISTS is_external_rider_system_enabled boolean DEFAULT true`);
    await db.execute(sql`ALTER TABLE platform_settings ADD COLUMN IF NOT EXISTS restaurants_enabled boolean DEFAULT false`);
    ...
    await db.execute(sql`UPDATE platform_settings SET is_external_rider_system_enabled = true WHERE is_external_rider_system_enabled IS NULL`);
