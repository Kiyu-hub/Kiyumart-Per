...
function toFiniteCoordinate(value: unknown, axis: "lat" | "lng"): number | null {
...
function timestampToMs(primary?: string | null, secondary?: string | null): number {
...
function getRiderLastUpdateMs(rider: Pick<RiderLocation, "timestamp" | "lastSeenAt">): number {
  return timestampToMs(rider.timestamp, rider.lastSeenAt || null);
}

function isRiderGpsFresh(
  rider: Pick<RiderLocation, "timestamp" | "lastSeenAt">,
  nowMs: number,
  timeoutMs = GPS_STALE_TIMEOUT_MS,
): boolean {
  const lastUpdateMs = getRiderLastUpdateMs(rider);
  if (!Number.isFinite(lastUpdateMs) || lastUpdateMs <= 0) return false;
  return nowMs - lastUpdateMs <= timeoutMs;
}

function isRiderEffectivelyOnline(
  rider: Pick<RiderLocation, "isOnline" | "onlineStatus" | "timestamp" | "lastSeenAt">,
  nowMs: number,
): boolean {
  return isRiderOnlineSnapshot(rider) && isRiderGpsFresh(rider, nowMs);
}
...
function dedupeRiderSnapshots(rows: RiderLocation[]): RiderLocation[] {
  const byRider = new Map<string, { rider: RiderLocation; orderIds: Set<string> }>();

  rows.forEach((entry) => {
    const normalized = normalizeRiderSnapshot(entry);
    const key = normalized.riderId || `${normalized.riderName.toLowerCase()}::${String(normalized.riderPhone || "").toLowerCase()}`;
    if (!key) return;

    const existing = byRider.get(key);
    if (!existing) {
      const orderIds = new Set<string>();
      if (normalized.orderId) orderIds.add(normalized.orderId);
      byRider.set(key, { rider: normalized, orderIds });
      return;
    }

    const currentTs = timestampToMs(existing.rider.timestamp, existing.rider.lastSeenAt || null);
    const incomingTs = timestampToMs(normalized.timestamp, normalized.lastSeenAt || null);
    const incomingIsNewer = incomingTs >= currentTs;
    const newer = incomingIsNewer ? normalized : existing.rider;
    const older = incomingIsNewer ? existing.rider : normalized;

    if (
      incomingIsNewer &&
      !normalized.orderId &&
      Number(normalized.activeOrderCount || 0) === 0 &&
      normalized.hasActiveOrder === false
    ) {
      existing.orderIds.clear();
    }
    ...
    const explicitCount = Math.max(
      Number(older.activeOrderCount || 0),
      Number(newer.activeOrderCount || 0),
      existing.orderIds.size,
    );
    merged.activeOrderCount = explicitCount;
    merged.hasActiveOrder = explicitCount > 0 || Boolean(merged.hasActiveOrder);
    merged.hasLocation = merged.latitude !== null && merged.longitude !== null;
    existing.rider = merged;
  });

  return Array.from(byRider.values()).map(({ rider, orderIds }) => {
    const count = Math.max(Number(rider.activeOrderCount || 0), orderIds.size);
    return {
      ...rider,
      activeOrderCount: count,
      hasActiveOrder: count > 0 || Boolean(rider.hasActiveOrder),
      hasLocation: rider.latitude !== null && rider.longitude !== null,
    };
  });
}
...
export default function RealTimeRiderMap({ forceMapboxGl = false }: RealTimeRiderMapProps) {
  ...
  const [gpsMonitorNowMs, setGpsMonitorNowMs] = useState(() => Date.now());
  ...
  useEffect(() => {
    ...
    const handleModeChanged = (event: Event) => {
      ...
      if (mode === "open_source") setPreferOpenSourceMap(true);
      else if (mode === "mapbox") setPreferOpenSourceMap(false);
    };
    ...
    gps:
      selectedRider?.latitude != null && selectedRider?.longitude != null
        ? {
            ...
            timestampMs: selectedRider.timestamp ? new Date(selectedRider.timestamp).getTime() : undefined,
          }
        : undefined,
  });
  ...
  const visibleStaleRiders = useMemo(
    () => visibleRiders.filter((rider) => !isRiderGpsFresh(rider, gpsMonitorNowMs)),
    [visibleRiders, gpsMonitorNowMs],
  );
  const visibleStaleRidersWithLocation = useMemo(
    () => visibleStaleRiders.filter((rider) => rider.latitude !== null && rider.longitude !== null),
    [visibleStaleRiders],
  );
  const visibleNoGpsActiveOrders = visibleRidersWithoutLocation.reduce(
  ...
  const mapPointsForFit = useMemo<Array<[number, number]>>(() => {
    ...
    return points;
  }, [selectedRiderOrder, trackSelectedOnly, visibleMapRiders]);
  ...
  const clusterBreakZoom = useMemo(() => {
    const riderCount = visibleMapRiders.length;
  ...
  }, [selectedOpenSourcePreset.maxZoom, selectedOpenSourcePreset.minZoom, visibleMapRiders.length]);
    ...
    usageSeverity >= 90
      ...
      : usageSeverity >= 80
        ...
        : usageSeverity >= 60
          ...
          : "text-emerald-700";
  const usageFactors = [
    visibleMapRiders.length > 35 ? "Many live riders increase tile downloads and camera updates." : null,
        ...
        <CardHeader className="py-3 pb-2 shrink-0">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2 justify-end">
              ...
              {visibleStaleRidersWithLocation.length > 0 && (
                <Badge variant="outline" className="text-orange-600 border-orange-300" data-testid="badge-stale-gps">
                  {visibleStaleRidersWithLocation.length} Stale GPS
                </Badge>
              )}
            ...
            <div className="mt-3 space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2 text-[11px]">
                ...
                {staleGpsRiderCount > 0 && (
                  <Badge variant="outline" className="border-orange-300 text-orange-700">Stale GPS: {staleGpsRiderCount}</Badge>
                )}
            ...
            >
              {isLoading ? (
                ...
                <>
                  {!mapBootstrapCenter ? (
                    ...
                    >
                            ...
                            <Popup>
                              <div className="p-2 min-w-[200px]">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {normalizeVehicleType(rider.vehicleType)} | {isRiderEffectivelyOnline(rider, gpsMonitorNowMs) ? "Online" : "Offline"}
                                </p>
                                {rider.orderNumber ? (
                      ...
                      {!usageSnapshot.freezeSecondaryLayers &&
                        !trackSelectedOnly &&
                        pendingOrders.filter((o) => o.deliveryLatitude != null && o.deliveryLongitude != null).map((order) => (
                        <Marker
                          key={order.id}
                  ...
                  {/* Empty state overlay */}
                  {visibleMapRiders.length === 0 && pendingOrders.length === 0 && (
                        ...
                        {visibleRiders.length > 0 ? (
                          <>
                            <p className="text-muted-foreground font-medium mb-2">
                              {visibleRiders.length} filtered {visibleRiders.length === 1 ? "rider" : "riders"} but no live GPS data
                            </p>
                            <p className="text-sm text-muted-foreground">
                              {visibleStaleRidersWithLocation.length > 0
                                ? `${visibleStaleRidersWithLocation.length} rider${visibleStaleRidersWithLocation.length === 1 ? "" : "s"} timed out (>15s heartbeat window).`
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-3 p-3 bg-muted rounded-lg">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {normalizeVehicleType(selectedRider.vehicleType)} | {selectedRiderIsOnline ? "Online" : "Offline"}
                          </p>
                        </div>
                        <Badge variant={selectedRiderIsOnline ? "secondary" : "outline"} className={selectedRiderIsOnline ? "text-emerald-700" : "text-slate-600"}>
                          {selectedRiderIsOnline ? "Online" : "Offline"}
                        </Badge>
                      </div>
              ...
              <Collapsible>
                <CollapsibleTrigger className="flex items-center justify-between w-full text-left">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline" className="border-amber-300 text-amber-700">
                      {visibleNoGpsActiveOrders} Active Order{visibleNoGpsActiveOrders !== 1 ? "s" : ""}
                    </Badge>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-3">
                    {visibleRidersWithoutLocation.map((rider) => (
                      <div
                        ...
                        className="rounded-xl border border-amber-200 bg-amber-50/80 p-3 dark:border-amber-800 dark:bg-amber-900/10"
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center justify-between gap-2">
                              <p className="truncate text-sm font-semibold">{rider.riderName}</p>
                              {(() => {
                                const orderCount = Math.max(0, Number(rider.activeOrderCount || (rider.orderId ? 1 : 0) || 0));
                                return (
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
                              ...
                              <span>{isRiderEffectivelyOnline(rider, gpsMonitorNowMs) ? "Online" : "Offline"}</span>
                              <span>
                                {rider.timestamp
                                  ? formatTimestamp(rider.timestamp)
                                  : rider.lastSeenAt
                                    ? formatTimestamp(rider.lastSeenAt)
                                    : "No location ping"}
                              </span>
                            </div>
                            <p className="mt-1 truncate text-xs text-muted-foreground">
                              {rider.orderNumber ? `Latest order #${rider.orderNumber}` : "No active order"}
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    Riders listed here are de-duplicated by rider identity. Order badges show active delivery count per rider.
                  </p>
                </CollapsibleContent>
              </Collapsible>
            </div>
          ...
          {selectedRider && (
            <div className="space-y-4">
              <div className="flex items-center gap-3 rounded-lg border bg-muted/40 p-3">
                ...
                <Badge variant={selectedRiderIsOnline ? "secondary" : "outline"}>
